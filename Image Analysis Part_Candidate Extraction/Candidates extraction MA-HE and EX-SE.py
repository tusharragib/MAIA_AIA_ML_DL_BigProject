...
def candidates_extraction(fundus,num,state):
	...
	cv.imwrite(f"MA-HE final result_{num}.png",result_ma_he)
	...
	# cv.imwrite(f"images/results/presentation/31-B Exudates reconstruction big exudates_{num}.png", result_exudates_big)
	...
	cv.imwrite(f"Exudates final result_{num}.png",  result_exudates)
...
state="bv already extracted"

for i in range(54): #starts from 0-53
	i=i+1
	if i<10:
		num='0'+f'{i}'
	else:
		num=f'{i}'
	fundus = cv.imread(f'{data_dir}/IDRiD_{num}.jpg')
	result_ma_he, result_exudates = candidates_extraction(fundus,num,state)
